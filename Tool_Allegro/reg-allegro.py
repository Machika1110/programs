...
# .pthファイルのリストを取得
pth_files = [file for file in os.listdir(current_directory) if file.endswith('.pth')]
print(pth_files[0])
...
def write_lammps_structure(file_path, structure):
    ...
    # in.reg-allegroを編集
    # in.reg-allegro_newを作成
    shutil.copy("in.reg-allegro", "in.reg-allegro-new")

    # in.reg-allegro_newの中身を編集
    # in.reg-allegro_newを読み込む
    with open("in.reg-allegro-new", "r") as f:
        lines = f.read()
    ...
    for i in reversed(atom_type):
        tmp += " " + i
        
    print(tmp)

    # 計算対象の系に含まれる元素に合わせて、pair_coeffを編集
    lines = lines.replace("pair_coeff", "pair_coeff   * * {}".format(pth_files[0]) + tmp)

    # in.reg-allegro_newへ書き込む
    with open("in.reg-allegro-new", "w") as f:
        f.write(lines)
...
def run_lammps(log_file):
    lammps_input_template = "in.reg-allegro-new"
...
dft_energies_train = []
dft_energies_test = []
lammps_energies_train = []
lammps_energies_test = []
ratio_inverse = int(1 / ratio_test)

for i in range(len(dft_energies)):
    if i+1 % ratio_inverse != 0:
        dft_energies_train.append(dft_energies[i])
        lammps_energies_train.append(lammps_energies[i])
    else:
        dft_energies_test.append(dft_energies[i])
        lammps_energies_test.append(lammps_energies[i])


# 回帰グラフの作成（訓練データ）
plt.figure(figsize=(10, 5))
plt.scatter(dft_energies_train, lammps_energies_train, label='Data Points')
plt.plot(dft_energies_train, dft_energies_train, color='red', linestyle='--', label='Ideal Fit')
...
plt.grid(True)
plt.show()
plt.savefig("reg-train.png")

# 回帰グラフの作成（テストデータ）
plt.figure(figsize=(10, 5))
plt.scatter(dft_energies_test, lammps_energies_test, label='Data Points')
plt.plot(dft_energies_test, dft_energies_test, color='red', linestyle='--', label='Ideal Fit')
plt.xlabel('DFT Energy per Atom (eV)')
plt.ylabel('LAMMPS Energy per Atom (eV)')
plt.title('Regression Plot of DFT vs. LAMMPS Energy of test data')
plt.legend()
plt.grid(True)
plt.show()
plt.savefig("reg-test.png")
...
with open("regression.txt", "w") as f:
    ...
    for i in range(len(dft_energies_train)):
        f.write("{} {}\n".format(dft_energies_train[i], lammps_energies_train[i]))

    f.write("\nenergy per atom [eV] of test data\n")
    f.write("DFT energy  LAMMPS energy\n")
    for i in range(len(dft_energies_test)):
        f.write("{} {}\n".format(dft_energies_test[i], lammps_energies_test[i]))
...
with open("dangerous_data.txt", "w") as f:
    ...
    for i in range(len(dft_energies)):
        if i+1 % ratio_inverse != 0 and abs(float(dft_energies[i])-float(lammps_energies[i])) > standard_error:
