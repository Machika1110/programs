...
input_condition = (
              ...
              'variable    hdnnpCutoff     equal  5\n'
              'variable    hdnnpDir        string "hdnnp"\n'
              ...
              'box          tilt large\n'
              'read_data    tmp.lmp\n'
              'pair_style   hdnnp ${hdnnpCutoff} dir ${hdnnpDir} showew no showewsum 100 resetew yes maxew 100000000\n'
              'pair_coeff   \n'
              'neighbor     2.0 bin\n'
...
# make in.file 
def make_inputfile():
    with open("in.reg-hdnnp", "w") as f:
        print(input_condition, file = f)
...
def write_lammps_structure(file_path, structure):
    ...
    # in.reg-hdnnpを編集
    # in.reg-hdnnp_newを作成
    shutil.copy("in.reg-hdnnp", "in.reg-hdnnp-new")

    # in.reg-hdnnp_newの中身を編集
    # in.reg-hdnnp_newを読み込む
    with open("in.reg-hdnnp-new", "r") as f:
        lines = f.read()
    ...
    lines = lines.replace("pair_coeff", "pair_coeff   * * {}".format(tmp))

    # in.reg-hdnnp_newへ書き込む
    with open("in.reg-hdnnp-new", "w") as f:
        f.write(lines)
...
def run_lammps(log_file):
    lammps_input_template = "in.reg-hdnnp-new"
    os.system(f"mpirun -np 16 lmp_mpi -in {lammps_input_template} -log {log_file}")

# input.data形式から.extxyz形式へ変換
...
### メイン処理 ###
make_inputfile()
convert_input_data_to_train_xyz(input_xyz, 'input.xyz')
structures = read_xyz("input.xyz")
dft_energies = []
lammps_energies = []
